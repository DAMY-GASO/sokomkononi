...
export default function WaitlistPage() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      // NOTE: submit via /contact/ until backend exposes a dedicated waitlist endpoint
      await contactApi.submit({
        name: "App Waitlist",
        email: email.trim(),
        message: "Waitlist signup for the SokoMkononi mobile app",
      });
      setSubmitted(true);
    } catch (err) {
      setError(
        ...
        <div className="dark-surface flex relative bg-[#101A2E] text-white flex-col justify-between p-8 md:p-10 lg:p-14 overflow-hidden">
          ...
          <div className="relative z-10 max-w-sm mx-auto text-center py-8 md:py-0">
            ...
            <p className="text-white/60 text-sm mt-3 leading-relaxed">
              {t("waitlist_panel_subtext") ||
                "Jiunge na waitlist yetu ili uwe wa kwanza kujua app ya SokoMkononi itakapopatikana."}
            </p>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 mt-8">
              <Link
                to="#"
                className="flex items-center gap-2 border border-white/20 rounded-md px-3 py-1.5 hover:bg-white/5 transition-colors"
              >
                <svg
                  viewBox="0 0 24 24"
                ...
                <span className="text-xs text-left leading-tight">
                  <span className="block text-white/50 text-[8px]">
                    {t("badge_get_it_on")}
                  </span>
                  <span className="block font-semibold text-white text-[10px]">
                    {t("badge_google_play")}
                  </span>
                </span>
              </Link>
            ...
            {!submitted ? (
              <>
                ...
                <p className="text-secondary text-body-sm mb-7 text-center">
                  {t("waitlist_form_subtext") ||
                    "Weka barua pepe yako ili upate taarifa za app."}
                </p>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  >
                    {loading
                      ? t("waitlist_submitting") || "Inajiunga..."
                      : t("waitlist_submit") || "Jiunge"}
                  </button>
                </form>
              ...
              <>
                ...
                <p className="text-secondary text-body-sm mb-7 text-center">
                  {t("waitlist_success_subtext") ||
                    "Tutakutumia ujumbe pindi app itakapokuwa tayari kupakuliwa."}
                </p>

                <Link
                  to="/"
                  className="w-full block text-center bg-[#E8A33D] hover:bg-[#B87A1F] text-[#101A2E] py-2.5 rounded-lg font-semibold text-btn transition-colors"
                >
                  {t("waitlist_back_home") || "← Rudi Nyumbani"}
                </Link>
              </>
