...
function HomePage() {
  ...
  const trendingProperties = [
    ...
    { title: "Kabati la Sebule, Kiembesamaki", region: "Zanzibar", price: "TSh 1,450,000", img: "/assets/trendings/zanzibar.jpg" },
    { title: "Shamba Tayari kwa Kilimo, Kilosa", region: "Morogoro", price: "TSh 15,000,000", img: "/assets/trendings/morogoro.jpg" },
    { title: "Shamba la Kilimo, Ismani", region: "Iringa", price: "TSh 18,500,000", img: "/assets/trendings/iringa.jpg" },
    { title: "Mbuzi wa Kienyeji, Ilongero", region: "Singida", price: "TSh 150,000", img: "/assets/trendings/singida.jpg" },
  ];
              ...
              {catOpen && (
                <div className="absolute left-0 mt-2 w-56 bg-[#182541] border border-white/10 rounded-lg shadow-xl py-2 z-50">
                  {categories.map((cat) => (
                    ...
                    >
                      {selectedLang === "sw" ? cat.name.sw : cat.name.en}
                    </Link>
                  ))}
                </div>
              )}
          ...
          {categories.map((cat, index) => (
            <Link key={cat.slug} to={`/kategoria/${cat.slug}`} className="bg-white rounded-lg overflow-hidden text-center border border-gray-100 hover:shadow-md transition-all hover:-translate-y-1 group">
              <div className="h-36 sm:h-40 overflow-hidden bg-[#F5F3EC]">
                <img
                  src={cat.img}
                  alt={selectedLang === "sw" ? cat.name.sw : cat.name.en}
                  loading="lazy"
                  className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                />
              </div>
              <div className="p-3">
                <h3 className="font-semibold text-gray-800 text-sm">{selectedLang === "sw" ? cat.name.sw : cat.name.en}</h3>
