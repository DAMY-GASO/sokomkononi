...
const getWaitingStatus = (lang) => ({
  ...
  notified: {
    label:
      lang === "sw"
        ? "Nafasi Wazi — Umetaarifiwa"
        : "Slot Open — You're Notified",
    color: COLORS.green,
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-4 mt-2 text-xs flex-wrap">
          ...
          <span style={{ color: "var(--text-muted)" }}>
            • {lang === "sw" ? "Umejiunga" : "Joined"}{" "}
            {timeAgo(entry.joinedAt, lang)}
          </span>
        </div>
        ...
        {entry.status === "notified" && (
          <div
            ...
            className="mt-3 rounded-lg border px-3 py-2 flex flex-col sm:flex-row items-center justify-between gap-2 text-center sm:text-left"
          >
            <span
        ...
        {(entry.status === "waiting" || entry.status === "notified") && (
          ...
          >
            <LogOut size={11} />{" "}
            {lang === "sw" ? "Ondoka kwenye Foleni" : "Leave Queue"}
          </button>
        )}
      </div>
    </div>
...
export default function WaitingListPage({
  ...
  const notifiedCount = entries.filter(
    (e) => e.status === "notified"
  ).length;
  ...
  return (
    ...
    >
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {/* ============================================================ */}
        {/* NOTIFIED NOTIFICATION — CENTERED */}
        {/* ============================================================ */}
        {notifiedCount > 0 && (
          <div
            style={{
              background: "rgba(47,109,79,0.1)",
              borderColor: "rgba(47,109,79,0.3)",
            }}
            className="rounded-xl border px-4 py-3 mb-4 flex flex-col items-center text-center gap-2"
          >
            ...
            >
              {lang === "sw" ? (
                <>
                  ...
                  {notifiedCount === 1 ? "iliyofunguka" : "zilizofunguka"} —
                  chukua hatua kabla muda haujaisha.
                </>
              ) : (
                <>
                  You have {notifiedCount} open slot
                  {notifiedCount === 1 ? "" : "s"} — take action before time runs
                  out.
                </>
              )}
            </p>
        ...
        {filtered.length === 0 ? (
          ...
          >
            <Clock3 size={48} className="mx-auto text-muted mb-3" />
            <h3
              style={{ color: "var(--text-primary)" }}
