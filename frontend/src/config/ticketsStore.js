...
export function addTicket({
  ...
  api.post("/tickets/", payload).then((raw) => {
...
export function addTicketMessage(id, { from = "admin", senderName = "Admin", text }) {
  ...
  saveAll(next);
  if (typeof id === "number") {
    api.post(`/tickets/${id}/messages/`, { text }).catch(() => {});
  }
  return next;
}
...
export function updateTicketStatus(id, status) {
  ...
  saveAll(next);
  if (typeof id === "number") {
    api.post(`/tickets/${id}/status/`, { status: STATUS_TO_API[status] || "OPEN" }).catch(() => {});
  }
  return next;
}
...
export function updateTicketPriority(id, priority) {
  ...
  saveAll(next);
  if (typeof id === "number") {
    api.post(`/tickets/${id}/priority/`, { priority: PRIORITY_TO_API[priority] || "MEDIUM" }).catch(() => {});
  }
  return next;
}
...
export function assignTicket(id, staffName, staffId) {
  const next = getTickets().map((t) =>
    t.id === id ? { ...t, assignedToName: staffName } : t
  );
  saveAll(next);
  if (typeof id === "number" && staffId) {
    api.post(`/tickets/${id}/assign/`, { staff_id: staffId }).catch(() => {});
  }
  return next;
}
