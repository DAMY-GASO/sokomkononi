...
import { notifyPaymentConfirmed } from "./notificationsStore.js";
...
export function addTransaction(txn) {
  ...
  // Malipo ya mwisho ya deal (sale/purchase) — fee types nyingine
  // (boost, leading, listing_fee, advertisement, bundle_purchase)
  // tayari zina emitter zake maalum zinazoitwa mahali husika.
  if (
    ...
    notifyPaymentConfirmed({
      dealId: entry.dealId,
      listingTitle: entry.property,
      amount: entry.amount,
      role: entry.type === "sale" ? "seller" : "buyer",
    });
  }
